...
class SmellParser:
    ...
    def extract_smells(output: str, diff_analysis: bool = False) -> Dict[str, int]:
        ...
        if not diff_analysis:
            matches = {
                "no_source_code": re.search(r":heavy_exclamation_mark: Packages with no source code URL \(⚠️⚠️⚠️\): (\d+)", output),
                ...
                "no_provenance": re.search(r":black_square_button: Packages without build attestation \(⚠️\): (\d+)", output),
            }
        else:
            matches = {
                "signature_changes": re.search(r":lock: Packages with signature changes \(⚠️⚠️⚠️\): (\d+)", output),
                "downgraded_dependencies": re.search(r":heavy_exclamation_mark: Downgraded packages \(⚠️⚠️\): (\d+)", output),
...
def run_tool_cli(project: str, version: str = None, diff_version: str = None, package_manager: str = "maven", gradual_report: bool = False, pnpm_scope: str = None) -> str:
    """
...
class TestSmellDetection:
    ...
    def test_static_maven_spoon(self, expected_outputs):
        ...
        assert actual_smells == expected_smells, \
    ...
    def test_static_maven_sbom_exe(self, expected_outputs):
        ...
        # Run tool
        actual_output = run_tool_cli(
            project="chains-project/sbom.exe",
            version="v0.14.1",
            package_manager="maven"
        )
    ...
    def test_diff_maven_spoon(self, expected_outputs):
        ...
        actual_output = run_tool_cli(
            project="INRIA/spoon",
            version="v11.1.1-beta-2",
            diff_version="v11.1.1-beta-9",
            package_manager="maven"
        )
