...
def fetch_and_cache_batch(commit_batch: List[Tuple[str, str, str, str]]) -> List[Dict]:
    ...
    query = "query(" + ", ".join([f"$nodeId{i}: ID!" for i in range(len(commit_batch))]) + ", $first: Int) {" + "\n".join(query_parts) + "}"
    ...
    if response and "data" in response:
        ...
        for i, (node_id, commit_sha, package, repo_name) in enumerate(commit_batch):
            ...
            if (node_key in response["data"] and 
                response["data"][node_key] is not None and 
                "associatedPullRequests" in response["data"][node_key]):
